(function() {
    ...
    test ( 'AeroGear.SyncEngine should support creation without the new keyword', function() {
        var engine = AeroGear.SyncEngine();
        ok( engine , 'Should be no problem not using new when creating' );
    });

    test( 'add document', function() {
        var engine = AeroGear.SyncEngine(), doc = { id: 1234, clientId: 'client1', content: { name: 'Fletch' } };
