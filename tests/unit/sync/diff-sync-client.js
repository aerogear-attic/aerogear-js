(function() {
    ...
    test( 'AeroGear.SyncClient should support creation without the new keyword', function() {
        var client = AeroGear.SyncClient( { serverUrl: 'ws://localhost:7777/sync' } );
        ok( client , 'Should be no problem not using new when creating' );
    });

    test( 'serverUrl is mandatory', function() {
        throws( function() { AeroGear.SyncClient(); } , Error, "'serverUrl' must be specified" );
    });
    ...
    //         start();
    //     };
    //     var client = AeroGear.SyncClient( {serverUrl: 'ws://localhost:7777/sync', onmessage: onmessage } );
    //     var documentId = uuid();
