...
( function() {
    ...
    var validAdapters = DataManager.validAdapters;

    module( "DataManager Creation with fallbacks", {
        setup: function() {

            for( var adapter in DataManager.validAdapters ) {
                if( adapter === "IndexedDB" || adapter === "WebSQL") {
                    delete DataManager.validAdapters[ adapter ];
                }
            }
        },
        teardown: function() {
            DataManager.validAdapters = validAdapters;
        }
    });
    ...
    test( "create IndexedDB - Fallsback to SessionLocal - name string", function() {
        ...
        equal( dm.createTest1 instanceof DataManager.adapters.SessionLocal, true, "Fellback to SessionLocal" );
    });
    ...
    test( "create IndexedDB and WebSQL - Fallsback to SessionLocal - name array", function() {
        ...
        equal( dm.createTest21 instanceof DataManager.adapters.SessionLocal, true, "Fellback to SessionLocal" );
        equal( dm.createTest22 instanceof DataManager.adapters.SessionLocal, true, "Fellback to SessionLocal" );
        equal( dm.createTest23 instanceof DataManager.adapters.Memory, true, "No Fallback should happen" );
    });
    ...
    test( "create - object", function() {
        ...
        equal( dm.createTest31 instanceof DataManager.adapters.Memory, true, "No Fallback should happen" );
        equal( dm.createTest32 instanceof DataManager.adapters.SessionLocal, true, "Fellback to SessionLocal" );
    });
    ...
    test( "create WebSQL - No Fallback - name string", function() {
        ...
        var dm = DataManager( { name: "createTest1", type: "WebSQL", settings: { fallback: false } } ).stores;
        equal( Object.keys( dm ).length, 1, "Single Store created" );
        equal( Object.keys( dm )[ 0 ], "createTest1", "Store Name createTest1" );
        equal( dm.createTest1 instanceof DataManager.adapters.WebSQL, true, "Didn't Fallback" );
    });
    ...
    test( "create WebSQL - No Fallback - name array", function() {
        ...
        equal( dm.createTest21 instanceof DataManager.adapters.WebSQL, true, "Didn't Fallback" );
        equal( dm.createTest23 instanceof DataManager.adapters.Memory, true, "No Fallback should happen" );
    });
    ...
    test( "create - object", function() {
        ...
        equal( Object.keys( dm )[ 1 ], "createTest32", "Store Name createTest32" );
        equal( dm.createTest31 instanceof DataManager.adapters.Memory, true, "No Fallback should happen" );
        equal( dm.createTest32 instanceof DataManager.adapters.WebSQL, true, "Didn't Fallback" );
    });
    ...
    test( "create IndexedDB - Fallsback to Memory - name string", function() {
        ...
        var dm = DataManager( { name: "createTest1", type: "IndexedDB", settings: { preferred: [ "Memory" ] } } ).stores;
        equal( Object.keys( dm ).length, 1, "Single Store created" );
        equal( Object.keys( dm )[ 0 ], "createTest1", "Store Name createTest1" );
        equal( dm.createTest1 instanceof DataManager.adapters.Memory, true, "Fellback to Memory" );
    });
    ...
    test( "create IndexedDB and WebSQL - Fallsback to Memory - name array", function() {
        ...
        equal( Object.keys( dm )[ 1 ], "createTest22", "Store Name createTest22" );
        equal( Object.keys( dm )[ 2 ], "createTest23", "Store Name createTest23" );
        equal( dm.createTest21 instanceof DataManager.adapters.Memory, true, "Fellback to Memory" );
        equal( dm.createTest22 instanceof DataManager.adapters.SessionLocal, true, "Fellback to SessionLocal" );
        equal( dm.createTest23 instanceof DataManager.adapters.Memory, true, "No Fallback should happen" );
    });
})( );
